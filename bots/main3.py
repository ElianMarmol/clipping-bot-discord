...
async def team_create(interaction: discord.Interaction, nombre: str, comision: float = 5.0):
    async with bot.db_pool.acquire() as conn:
        ...
        payment = await conn.fetchrow("SELECT * FROM payment_methods WHERE discord_id = $1", interaction.user.id)
        ...
        existing_team = await conn.fetchrow("SELECT * FROM teams WHERE owner_id = $1", interaction.user.id)
        if existing_team:
            await interaction.response.send_message("⚠️ Ya tenés un equipo creado.", ephemeral=True)
            return
...
async def team_join(interaction: discord.Interaction, codigo: str):
    async with bot.db_pool.acquire() as conn:
        team = await conn.fetchrow("SELECT * FROM teams WHERE invite_code = $1", codigo)
        if not team:
            await interaction.response.send_message("❌ Código inválido. Verificá que sea correcto.", ephemeral=True)
            return

        # Evitar que el owner se una como miembro
        if team["owner_id"] == interaction.user.id:
            await interaction.response.send_message("⚠️ No podés unirte a tu propio equipo.", ephemeral=True)
            return
        ...
        if existing:
            await interaction.response.send_message("ℹ️ Ya sos miembro de este equipo.", ephemeral=True)
            return
...
async def team_info(interaction: discord.Interaction):
    async with bot.db_pool.acquire() as conn:
        team = await conn.fetchrow("SELECT * FROM teams WHERE owner_id = $1", interaction.user.id)
        if not team:
            ...
            """, interaction.user.id)
            if not member_of:
                await interaction.response.send_message("❌ No pertenecés a ningún equipo.", ephemeral=True)
                return
            team = member_of
...
async def team_edit_commission(interaction: discord.Interaction, nueva_comision: float):
    async with bot.db_pool.acquire() as conn:
        team = await conn.fetchrow("SELECT * FROM teams WHERE owner_id = $1", interaction.user.id)
        if not team:
            await interaction.response.send_message("❌ No tenés ningún equipo creado.", ephemeral=True)
            return
...
async def team_new_inv(interaction: discord.Interaction):
    async with bot.db_pool.acquire() as conn:
        ...
        await conn.execute("UPDATE teams SET invite_code = $1 WHERE id = $2", new_invite, team['id'])
